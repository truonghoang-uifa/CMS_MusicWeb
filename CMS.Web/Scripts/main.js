...
function closeDatCauHoiPopup() {
    $("#datcauhoi_popup").css('visibility', 'hidden').animate({ opacity: 0 }, 100);
}
function datCauHoi() {
    if ($('#inpHoTen').val() === ''
        || $('#inpEmail').val() === ''
        || $('#inpChuDe').val() === ''
        || $('#inpNoiDungCauHoi').val() === '') {
        return;
    }
    $.post("/api/cauhoi/datcauhoi",
        {
            NguoiHoi: $('#inpHoTen').val(),
            DienThoai: $('#inpSdt').val(),
            Email: $('#inpEmail').val(),
            ChuDeCauHoiId: $('#inpChuDe').val(),
            NoiDungCauHoi: $('#inpNoiDungCauHoi').val()
        }, function (data, status) {
            closeDatCauHoiPopup();
            $('#datcauhoithanhcong_popup').css({
                "opacity": "1",
                "visibility": "visible"
            });
            setTimeout(function () {
                $("#datcauhoithanhcong_popup").css('visibility', 'hidden').animate({ opacity: 0 }, 100);
            }, 2000);
        });
}